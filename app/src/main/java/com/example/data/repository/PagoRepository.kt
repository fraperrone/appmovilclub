...
//            val db = dbHelper.readableDatabase
//
//            // Consulta optimizada: obtener los IDs de clientes que tienen al menos un pago
//            val query = """
...
//            db.close()
//
//            // Filtramos los que no tienen pagos
//            val clientesSinPagos = clientes.filter { it.id !in clientesConPagos }
//
//            // Creamos objetos ClienteConDeuda con valores por defecto
...
class PagoRepository(context: Context) {
    ...
    fun obtenerClientesConCuotaVencida(): List<ClienteConDeuda> {
        ...
        for (cliente in clientes) {
            ...
            if (ultimoPago != null) {
                try {
                    // Intentamos parsear "yyyy-MM-dd", si falla usamos "yyyy-MM-dd HH:mm:ss"
